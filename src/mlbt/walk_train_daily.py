...
def walk_train_daily(
    ...
    for win_idx, (ws, we) in enumerate(windows):
        ...
        # Backtest on the window using the same dataset slice
        test_df = df.loc[ws:we]
        ...
        rows.append({
            ...
            "beats_sharpe": (net.get("sharpe", 0) or 0) > (spy.get("sharpe", 0) or 0),
            "beats_calmar": net_calmar > spy_calmar,
            "dd_better": (net.get("max_drawdown", 0) or -1) > (spy.get("max_drawdown", 0) or -1),
    ...
    if not out_df.empty:
        ...
        n = len(out_df)
        n_bs = int(out_df["beats_sharpe"].sum())
        n_bc = int(out_df["beats_calmar"].sum())
        n_dd = int(out_df["dd_better"].sum())
        print(f"\nbeats_sharpe: {n_bs}/{n}  beats_calmar: {n_bc}/{n}  dd_better: {n_dd}/{n}")
        # success criterion
        passes = (max(n_bs, n_bc) >= int(n * 0.8)) and (n_dd == n)
        print(f"meets 4/5+ AND dd_better in EVERY window? {passes}")
    return out_df
